...
{/* Header */}
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <input type="text" id="name" name="name" className={`${inputClasses} ${errors.name ? errorInputClasses : ''}`} value={formData.name} onChange={handleChange} placeholder="Enter your full name" disabled={isSubmitting} />
        {errors.name && <p className="mt-1 text-xs text-red-300">{errors.name}</p>}
      </div>

      <div>
        <label htmlFor="email" className="block text-sm font-semibold text-gray-200">Email Address</label>
        <input type="email" id="email" name="email" className={`${inputClasses} ${errors.email ? errorInputClasses : ''}`} value={formData.email} onChange={handleChange} placeholder="Enter your email" disabled={isSubmitting} />
        {errors.email && <p className="mt-1 text-xs text-red-300">{errors.email}</p>}
      </div>

      <div>
        <label htmlFor="password" className="block text-sm font-semibold text-gray-200">Password</label>
        <input type="password" id="password" name="password" className={`${inputClasses} ${errors.password ? errorInputClasses : ''}`} value={formData.password} onChange={handleChange} placeholder="Enter your password" disabled={isSubmitting} />
      ...
      <div>
        ...
        <input type="password" id="confirmPassword" name="confirmPassword" className={`${inputClasses} ${errors.confirmPassword ? errorInputClasses : ''}`} value={formData.confirmPassword} onChange={handleChange} placeholder="Confirm your password" disabled={isSubmitting} />
